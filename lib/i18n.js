...
const EN_DICT = {
  ...
  // ゲストモードの案内
  // ラウンド50：マイ予約の実際の検索条件（お名前＋電話番号、二要素確認）に合わせて文言を修正。
  // 旧文言（電話番号のみ）は下のマイ予約画面のヒント文言と食い違っていたための修正。
  'LINEなしでご予約いただけます。ご予約の確認・変更・キャンセルは「マイ予約」からお名前・電話番号で検索できます': 'You can book without LINE. To check, change, or cancel your booking, use "My Bookings" and search by name and phone number',
  ...
  // サーバー側の内部エラー（GASタイムアウト等の技術的な文字列）をお客様向けに差し替える際の文言
  // （friendlyServerError参照、ラウンド50での指摘対応）
  '一時的な問題が発生しました。しばらくしてから、もう一度お試しください。繰り返し発生する場合はお電話にてご連絡ください。': 'A temporary problem occurred. Please wait a moment and try again. If this keeps happening, please call us.',
  ...
  // 送信直後にLIFFのWebViewごと落ちる／回線が切れる等で結果を見られないまま次に開いた場合、
  // 実際には成功していたと確認できた時に見せる案内（checkPendingSubmitIfAnyのコメント参照）。
  '前回、通信が不安定になり結果が表示されないまま終了しましたが、ご予約は正常に完了していました。': 'Your connection was interrupted before you could see the result last time, but your booking was completed successfully.',
  ...
  // マイ予約
  // ラウンド50：見出しが「電話番号でご予約を確認」のままで、直下のヒント文・実際の検索条件
  // （お名前＋電話番号、二要素確認）と食い違っていたための修正。旧文言に対応していた
  // 「LINEをご利用でないため、ご予約時に入力した電話番号でご予約を検索します。」は、コード側に
  // 対応する呼び出しが既に無い（電話番号のみ版から名前必須版に切り替わった際の残骸）ため削除。
  '📞　お名前・電話番号でご予約を確認': '📞  Find My Booking by Name and Phone Number',
  ...
  // LINEでの自動確認が失敗した場合の行き止まり解消用フォールバックボタン（ラウンド51）
  '📞　お名前・電話番号で確認する': '📞  Check with Name and Phone Number Instead',
  '現在、確定しているご予約はございません。': 'You have no upcoming bookings.',
  // LINEでの自動確認が「0件」という成功応答（本人確認失敗と見分けが付かない）で終わった場合の案内
  // （上のフォールバックボタンと同時に追加、ラウンド51）
  '見つからない場合は、ご予約時のお名前・電話番号でも確認できます': "If you don't see it here, you can also check with the name and phone number you used when booking.",
  ...
  // マイ予約：自分のキャンセル待ち登録一覧（審判団ラウンド46で新設、ラウンド47で英訳漏れを解消。
  // これまでキャンセル待ち登録関連の一部の文言は翻訳対象外としてきたが、この一覧は「マイ予約」画面
  // 自体の一部（優先翻訳範囲）に組み込まれているため、他の「マイ予約」文言と同じ扱いにする）
  '🕒 キャンセル待ち登録中': '🕒 On the Waitlist',
  '満席のためキャンセル待ちにご登録いただいた内容です。空きが出た場合はご登録の連絡先へお知らせします（先着順のため必ずご案内できるとは限りません）。': "You're registered on the waitlist for this fully-booked date. We'll contact you using the details you registered if a spot opens up (first-come, first-served — this does not guarantee a spot).",
  '取り消す': 'Withdraw',
  'このキャンセル待ち登録を取り消しますか？': 'Withdraw this waitlist entry?',
  '✕ 取り消しました': '✕ Withdrawn',
  '取り消しに失敗しました。お手数ですがお電話にてご連絡ください。': "Failed to withdraw. We're sorry for the inconvenience — please call us.",
  // ラウンド48：スタッフが管理画面で同じキャンセル待ちを削除済み、または既に空きの案内が済んでいた
  // 場合に「✕ 取り消しました」と区別して出す文言（execWaitlistCancelのalreadyGone分岐参照）。
  'この登録は既に終了しています（空きのご案内が済んでいるか、既に取り消し済みです）': 'This entry is no longer active (either we already contacted you about an opening, or it was already withdrawn).',
