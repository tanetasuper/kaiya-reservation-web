...
function ManualContent({ s, loadErr }) {
  ...
  // 「満席」は「席」前提の言葉のため、countUnitが「台」（レンタカー・レンタサイクル・「リフト」呼びの
  // 整備工場等、資産・車両を数える業態）の店舗にそのまま案内すると、実際には無い「席」という言葉だけが
  // マニュアルに残ってしまう（ラウンド52・業種経営者陣視点レビューでの指摘、pages/index.jsのcapacityText
  // と同じ置き換えルール）。countUnit既定「名」の業態（perStaff系含む）は判断が割れる主観的な言い換えを
  // 強制せず、従来通り「満席」のまま。
  const fullWord = (s && s.countUnit === '台') ? '満車' : '満席'
  ...
  const catalog = s ? [
    ...
    { name: 'キャンセル待ち', on: waitlistOn, desc: `${fullWord}の日にお客様がキャンセル待ちに登録できるようにする` },
      ...
      <div className="wrap">
        ...
        <section className="card" id="add">
          ...
          <div className="callout warn">
            <span className="icon" aria-hidden="true">⚠️</span>
            <div>
              {fullWord}・休業日などの制限に引っかかると、<b>「強制登録（休業日を無視）」</b>というチェックボックスを使うよう案内が出ることがあります。これは<b>本来の制限を無視して無理に登録する</b>機能です。常連のお得意様の特別対応や、システムの都合と実際のお店の状況がズレている場合にのみ使ってください。使う前に一声、店長に確認するのがおすすめです。
            </div>
          </div>
        </section>
        ...
        {waitlistOn && (
          <>
            ...
            <section className="card" id="waitlist">
              ...
              <h2>キャンセル待ちに対応する</h2>
              <p>{fullWord}の日に、お客様がキャンセル待ちに登録できます。登録内容は<span className="ui">通知</span>タブの一覧に、日付ごとにまとめて表示されます（日付をタップした先の詳細画面ではありません）。{/* スタッフ目線レビューでの指摘：以前はカレンダーの日付詳細画面に表示されると案内していたが、実際の機能は日付を問わず全件を横断表示する通知タブの中にある */}</p>
        ...
        <section className="card" id="trouble">
          ...
          <table className="legend">
            <tbody>
              ...
              <tr><td>お客様から「予約できない」と連絡があった</td><td>その日が休業日・{fullWord}・停止枠になっていないか、日付の詳細画面で確認してください。</td></tr>
